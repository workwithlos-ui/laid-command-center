...
function splitPrompt(prompt) {
  if (prompt && typeof prompt === 'object' && 'system' in prompt && 'user' in prompt) {
    return {
      system: String(prompt.system || 'Return valid JSON only. Never use em dash characters.'),
      user: String(prompt.user || ''),
    };
  }
  return {
    system: 'Return valid JSON only. Use real sources when asked. Never use em dash characters.',
    user: String(prompt || ''),
  };
}

async function callResponses({ apiKey, prompt, schema, allowSearch }) {
  const prepared = splitPrompt(prompt);
  const body = {
    model: DEFAULT_MODEL,
    input: [
      {
