...
Location: Orlando, FL
Target: Home services (roofing, HVAC, plumbing), DTC brands, agencies

We have a React app (React 19 + TypeScript + Vite + Tailwind + shadcn/ui) deployed at:
https://kfj7fz4jp7fdu.kimi.page/#/feed

The app has 6 tabs: Dashboard, Feed (90 content pieces), Assets (15 deliverables), Tracker (20 prospects), Swarm, Settings.
...
**Content structure (PRO tone):**
Hook (money angle) → News (what dropped) → The 3 Things (ranked by ROI) → Tactical Commands (copy-paste) → Money Line (specific $ result) → Ignore This And... (consequence) → CTA (DM [KEYWORD])

**Content structure (BEGINNER tone):**
Hook (relatable, warm) → What It Is (simple explanation) → Why Beginners Love This → Analogy (real-world comparison) → Your First 4 Steps → Your First Win → Encouragement → CTA (DM [KEYWORD])
...
- Scores stories 1-10 by business relevance
- Auto-categorizes into keyword buckets
- Drafts content in both pro + beginner tones
- Queues for human review before import
...
2. Write content in multiple tones
3. Design carousels with hand-crafted templates
4. Track performance and learn from feedback
5. Get smarter every week

### Agent Architecture

**Director Agent** (Orchestrator)
- Role: System coordinator
- Objective: Route tasks, manage workflow, track performance
- Memory: Which topics perform best, which tones get most DMs, which assets are most requested
- Action: Triggers other agents, compiles reports, suggests optimizations

**Research Agent** (News Hunter)
- Role: AI industry intelligence
- Sources: 15 monitored sources (web + RSS)
- Scoring: Business relevance 1-10, money potential, ease of use
- Output: Ranked story list with auto-categorization
- Memory: Past stories covered, what resonated, what flopped

**Content Pro Agent** (Aggressive Money Tone)
- Role: Write high-energy, money-focused content
- Style: Julian Goldie + Sabrina Ramonov hybrid
- Structure: Hook → News → 3 Things → Commands → Money Line → Consequence → CTA
- Memory: Which hooks got DMs, which commands were copied

**Content Beginner Agent** (Warm Human Tone)
- Role: Write friendly, approachable content for non-technical owners
- Style: Mikey NoCode + knowledgeable friend over coffee
- Structure: Hook → What It Is → Why Beginners → Analogy → 4 Steps → First Win → Encouragement → CTA
- Memory: Which analogies landed, which steps were too complex

**Hook Agent** (Viral Hook Specialist)
- Role: Write 5 hook variations for every story, pick best based on past performance
- Patterns: Comparison hooks, number hooks, money hooks, fear hooks, curiosity hooks
- Memory: Hook → DM conversion rate per pattern

**Design Agent** (Carousel Builder)
- Role: Generate pixel-perfect carousel images from hand-coded templates
...
- Metrics: Posts made, DMs received per keyword, assets downloaded, calls booked, deals closed
- Memory: Week-over-week trends, best performing topics, optimal posting times
- Action: Weekly report to Director with optimization suggestions

**Sales Agent** (Pipeline Manager)
- Role: Manage prospect interactions and follow-ups
- Input: Prospect tracker data, DM responses, call outcomes
- Action: Suggest follow-up messages, track stage movements, flag hot leads
- Memory: Which icebreakers work, which follow-up timing converts

### Self-Improvement Loop

```
Week 1: Swarm produces content → Human posts → Tracks DMs
Week 2: Analytics Agent reports: "LEADS keyword got 8 DMs, TEAM got 0"
Week 3: Director shifts: 40% LEADS content, 10% TEAM, rest redistributed
Week 4: Hook Agent sees comparison hooks outperform by 3x → prioritizes that pattern
Week 5: Design Agent learns image posts get 2.5x engagement → generates more carousels
Week 6: Full cycle repeat → every metric improves 10-20%
```

### Memory System

Every agent maintains a memory file:
```json
{
  "agent": "Content Pro",
  "learnings": [
    "Comparison hooks (X just killed Y) outperform by 3x",
    "Posts with specific dollar amounts get 2x more DMs",
    "'If you ignore this' line increases engagement 40%",
    "Thursday 8-9 AM is optimal posting window"
  ],
  "best_performing": ["LEADS", "CREATE", "AUTOMATE"],
  "worst_performing": ["TEAM", "CODE"],
  "hook_templates_proven": ["X just killed Y", "$Z tool does W", "N ways to..."]
}
```
...
**Every piece MUST:**
- Be based on REAL, VERIFIED AI news (last 7 days)
- Include exact date and source
...
**PRO hook patterns:**
- "[Tool] just killed [old way]"
- "$[amount] [tool] vs $[amount] [competitor]"
- "[Number] ways [tool] pays for itself in [time]"
...
When I say "build", here's the tech process:
1. Content → Write to src/data/laid_content.json
2. Assets → Write to src/data/laid_assets.json
3. Prospects → Write to src/data/laid_prospects.json
4. Build → npm run build (in /mnt/agents/output/app/)
5. Deploy → Deploy dist/ folder
6. Verify → Check live URL, confirm changes

When I say "research", you must:
1. Use browser_visit to check actual sources
2. Use web_search with multiple queries
3. Cross-reference at least 2 sources
4. Only include verified facts with dates

When I say "add carousel template", you must:
1. Study real carousels from top creators
...
Report back with specific, actionable output. No generic advice. No homework for me. You execute, I approve or redirect.
